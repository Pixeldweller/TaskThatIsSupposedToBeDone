...
class Projects(StartPage):
    ...
    def default_view(self, feedbackmessage =''):
        ...
        pro_dict['details'] = self.db.readFile('project_employee_data.json')
    ...
    @cherrypy.expose
    def edit(self, p_id, reloaded = False, num=-1, title='-Neuer Titel-', desc='-Neue Beschreibung-', startdate='',duration = 1, budget= 0.0, customer = 0):
    ...
    def update_project(self, num, number, title, desc, startdate, duration, budget,employee,customer):
        ...
        empl_dict = self.db.readFile('employee.json')
        data = []
        lastEmplId = None
        lastEntry = None
        # incoming ex. " '1-pw0': 1 "
        ...
        for entry in employee:
            ...
            if lastEmplId is not info[0]:
                ...
                self.db.findId('employee.json', lastEntry['employee_id'])
                lastEmplId = info[0]
    ...
    def convert_employee_info(self, args):
        data = []
        for entry in args:
            time = args[entry]
            if float(time) <= 0:
