...
class CustomersPage(StartPage):

    def __init__(self, view, db):
        StartPage.__init__(self,view,db)

    def default_view(self, feedbackmessage=''):
        dict = self.get_customers()
        dict['msg'] = feedbackmessage
        return self.view.renderTemplateWithDictionary("customers/c_table.mako", data_op=dict)
    ...
    def add(self, id, name, number, contact, address, email, phn):
        ...
        try:
            if data is not None:
                ...
                return self.default_view('Hinzufügen erfolgreich.')
    ...
    def delete_customer(self, id):
        ...
        for packet in dictionary_project['data']:
            if packet['customer'] == int(id):
                return self.default_view('Dieser Datensatz wird noch in einem Projektobjekt referenziert und kann daher nicht gelöscht werden.')
        ...
        return self.default_view('Der Datensatz wurde gelöscht.')
