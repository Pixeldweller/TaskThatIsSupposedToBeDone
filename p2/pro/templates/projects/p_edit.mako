...
<%block name="content">
    ...
    <form id="addForm" action ="/projects/add/" method="GET" >
        ...
        <div>
            Laufzeit in Wochen: <input id="duration" onchange="window.location = '/projects/edit/'+getValue('p_id')+'/TRUE/'+getValue('p_num')+'/'+getValue('title')+'/'+getValue('desc')+'/'+getValue('startdate')+'/'+getValue('duration')+'/'+getValue('budget')+'/'+getValue('customer')+'/#duration'" type="number" min="0.00" name="duration" required value="${data_o['duration']}"/>
        </div>
        <div>
