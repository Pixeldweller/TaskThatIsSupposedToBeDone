...
//------------------------------------------------------------------------------
    ...
    handleEvent_p(event_opl) {
        ...
        if (event_opl.target.tagName.toUpperCase() == "TD") {
            ...
            if (el_o.title == "fehler") {
                let swap_bug_done_buttons = document.querySelectorAll(".needsBehoben");
                let swap_bug_new_buttons = document.querySelectorAll(".needsErkannt");
                for (i = 0; i < swap_bug_done_buttons.length; i++) {
                    swap_bug_done_buttons[i].disabled = elx_o.title !== "BEHOBEN";
                }
                for (i = 0; i < swap_bug_new_buttons.length; i++) {
                    swap_bug_new_buttons[i].disabled = elx_o.title !== "ERFASST";
                }
            }
