...
//------------------------------------------------------------------------------
        ...
        if (id_spl === -1) {
            ...
            requester_o.request_px(path_s,
                function (responseText_spl) {
                    ...
                    let newEntryBool = id_spl < 0;
                    this.doRender_p(object_type, data_o, newEntryBool, target_path);
                }.bind(this),
                function (responseText_spl) {
    ...
    configHandleEvent_p(object_type, addNewEntry, target_path) {
        ...
        if (form_o != null) {
            form_o.target = addNewEntry ? "new" : "edit";
            if (main_o != null) {
            ...
            if (!requiredInputNotFilled) {
                ...
                if (form.target !== "new") {
                    ...
                    requester_o.post_px("/" + target_path + "/", formData, function (response) {
                        if (el_o.title == 'event') {
                            APPUTIL.es_o.publish_px("app.cmd", ["events"]);
                        } else {
                            APPUTIL.es_o.publish_px("app.cmd", [el_o.title, undefined, undefined, 'saved']);
                        }
                    }, function () {
